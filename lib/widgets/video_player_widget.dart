...
class _NativeVideoPlayerState extends State<NativeVideoPlayer> {
  ...
  Future<void> _initializePlayer() async {
    ...
    if (widget.streamData.referer.isNotEmpty) {
      headers['Referer'] = widget.streamData.referer;
      try {
        headers['Origin'] = Uri.parse(widget.streamData.referer).origin;
      } catch (e) {
        if (widget.streamData.referer.endsWith('/')) {
          headers['Origin'] = widget.streamData.referer
              .substring(0, widget.streamData.referer.length - 1);
        } else {
          headers['Origin'] = widget.streamData.referer;
        }
      }
  ...
  void _playerListener() {
    ...
    if (widget.streamData.recapStart != null &&
        widget.streamData.recapEnd != null) {
      if (pos >= widget.streamData.recapStart! &&
          pos < widget.streamData.recapEnd!) {
        currentSkip = 'Recap';
      }
    }
